...
AppleFeatures = appleFeat;
BurgerFeatures = burgerFeat;
...
numapple = size(AppleFeatures,1);
numburger = size(BurgerFeatures,1);
...
% appleSet =
% double(cell2mat(AppleFeatures(idxapple(1:numSample/numSets))))'; % SURF
% features
% burgerSet =
% double(cell2mat(BurgerFeatures(idxburger(1:numSample/numSets))))'; % SURF
% features
appleSet = double(AppleFeatures(idxapple(1:numSample/numSets), :));
burgerSet = double(BurgerFeatures(idxburger(1:numSample/numSets), :));
...
pred = PerceptronBatchNormalized(F_Train,T_Train,F_Test, 1000);
